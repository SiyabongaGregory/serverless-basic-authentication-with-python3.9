...
module.exports = class SetupBasicAuthentication {
  constructor(serverless, options) {
    ...
    this.hooks = {
      ...
      "after:package:createDeploymentArtifacts":
        this.removeAuthorizer.bind(this),
  ...
  removeAuthorizer() {
    this.serverless.cli.consoleLog(
      `Basic Authentication: ${chalk.yellow(
        "Removing Symlink for Basic Authenticator"
      )}`
    );
    fs.unlinkSync(
  ...
  addAuthFileToPackage() {
    ...
    this.serverless.cli.consoleLog(
      `Basic Authentication: ${chalk.yellow(
        "Adding Symlink for Basic Authenticator"
      )}`
    );
    ...
    this.serverless.package.include.push(
  ...
  injectBasicAuthFunction() {
    this.serverless.cli.consoleLog(
      `Basic Authentication: ${chalk.yellow(
        "Adding function for Basic Authenticator"
      )}`
    );
    ...
    Object.keys(this.serverless.service.functions).forEach((functionName) => {
      ...
      Object.keys(fnctn.events).forEach((fnctnEvent) => {
        ...
        if (
          !(
            "http" in
            this.serverless.service.functions[functionName].events[fnctnEvent]
          )
        ) {
          return;
        }
        ...
        if (
          ...
          this.serverless.service.functions[functionName].events[fnctnEvent]
            .http.private === true &&
          this.serverless.service.functions[functionName].events[fnctnEvent]
            .http.authorizer == null
        ) {
          this.serverless.service.functions[functionName].events[
            fnctnEvent
          ].http.authorizer = {
  ...
  configureApiGatewayKeySource() {
    ...
    if (template.Resources.ApiGatewayRestApi != null) {
      this.serverless.cli.consoleLog(
        `Basic Authentication: ${chalk.yellow(
          "Configuring Api Gateway for Basic Authenticator"
        )}`
      );
      template.Resources.ApiGatewayRestApi.Properties.ApiKeySourceType =
        "AUTHORIZER";
    }
  }
};
